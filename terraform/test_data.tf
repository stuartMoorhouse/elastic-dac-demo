  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      load_test_data() {
        ...
        # Create index with proper ECS mapping for network packet capture
        echo "Creating logs-network index with ECS mapping in $${CLUSTER_NAME}..."
        curl -k -X PUT "$${ES_ENDPOINT}/logs-network.packet_capture-default" \
...
'
        ...
        # Refresh the index
        curl -k -X POST "$${ES_ENDPOINT}/logs-network.packet_capture-default/_refresh" \
          -u "$${ES_USERNAME}:$${ES_PASSWORD}"
        ...
        TOTAL_DOCS=$(curl -sk -X GET "$${ES_ENDPOINT}/logs-network.packet_capture-default/_count" \
          -u "$${ES_USERNAME}:$${ES_PASSWORD}" \
          -H "Content-Type: application/json" | jq -r '.count')
        
        echo "Total documents in logs-network.packet_capture-default index: $${TOTAL_DOCS}"
        
        # Count malicious traffic that should trigger the rule
        MALICIOUS_COUNT=$(curl -sk -X GET "$${ES_ENDPOINT}/logs-network.packet_capture-default/_count" \
          -u "$${ES_USERNAME}:$${ES_PASSWORD}" \
        -H "Content-Type: application/json" \
        -d '{
          "query": {
...
output "test_data_info" {
  value = {
    index_name = "logs-network.packet_capture-default"
